...
| **Automotive Platforms** | **HMI & GUI Development (Qt/AAOS)** | Understands basic UI layouts (QML/Android XML); writes simple static screens; understands MVC/MVVM patterns. | Develops functional Qt widgets/QML screens or Android AAOS user interfaces; binds UI controls to backend service properties. | Creates custom QML components and custom Android Car App layouts; integrates screens with SOME/IP and Android VHAL; optimizes GPU rendering. | Architects multi-screen cockpit displays (Cluster, Infotainment, HUD); develops custom Qt Quick plugin layers; optimizes UI boot performance. | Defines cockpit HMI style guides and tech stack (Qt vs AAOS/QML); designs modular, reusable UI frameworks across multiple vehicle models. |
